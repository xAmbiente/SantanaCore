...
namespace Santana
{
    public class Config
    {
        ...
        static Config()
        {
            ...
            if (Instance == null)
            {
                Console.WriteLine("[Bootstrap] AuthServer.hjson could not be deserialized; falling back to a freshly generated configuration");
                Instance = new Config();
                Instance.Save();
            }

            Instance.API ??= new APIConfig();
            Instance.AuthAPI ??= new AuthAPIConfig();
            Instance.Database ??= new DatabasesConfig();
            Instance.Database.Auth ??= new DatabasesConfig.DatabaseConfig();
            Instance.AuthAPI.BlockedHWIDS ??= new string[0];
