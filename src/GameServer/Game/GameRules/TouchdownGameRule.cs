    ...
    internal sealed class TouchdownGameRule : GameRuleBase
    {
        ...
        public override void Update(TimeSpan delta)
        {
            ...
            try
            {
                ...
                {
                    var scoreLimit = inFirstHalf ? Room.Options.ScoreLimit / 2 : Room.Options.ScoreLimit;
                    var trigger = inFirstHalf ? GameRuleStateTrigger.StartHalfTime : GameRuleStateTrigger.StartResult;
                    if (teams.Values.Any(team => team.Score >= scoreLimit) && StateMachine.CanFire(trigger))
                        StateMachine.Fire(trigger);
                    var roundTimeLimit = TimeSpan.FromMilliseconds(Room.Options.TimeLimit.TotalMilliseconds / 2);
                    if (RoundTime >= roundTimeLimit && StateMachine.CanFire(trigger))
                        StateMachine.Fire(trigger);
                }
                if (!_celebrating)
                    return;
        ...
        public void OnScoreGoal(Player plr, LongPeerId scoreTarget)
        {
            ...
            {
                plr.AchieveMission.TD++;
                plr.DailyMission.TD++;
                Room.Broadcast(new ScoreGoalAckMessage(_currentHolder));
            }
            _currentHolder = 0;
            _previousHolder = 0;
