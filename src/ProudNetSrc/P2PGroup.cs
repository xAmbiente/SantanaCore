namespace ProudNetSrc
{
  ...
  public class P2PGroup
  {
    ...
    public void Join(uint hostId)
    {
      ...
      {
        _server.Configuration.Logger?.Debug("Member {HostId} enrolled into mesh {GroupHostId}", hostId, HostId);
        ...
        foreach (var member in _members.Values.Where(member => member.HostId != hostId))
        {
          ...
          {
            _members.TryRemove(member.HostId, out _);
            continue;
          }
    ...
    public void Leave(uint hostId)
    {
      ...
      {
        _server.Configuration.Logger?.Debug("Member {HostId} dropped out of mesh {GroupHostId}", hostId, HostId);
