...
namespace ProudNetSrc
{
  public class P2PGroupManager : IReadOnlyDictionary<uint, P2PGroup>
  {
    ...
    public P2PGroup Create(bool allowDirectP2P)
    {
      ...
      _server.Configuration.Logger?.Debug("Allocated mesh {HostId}, direct links permitted: {AllowDirectP2P}", group.HostId,
          allowDirectP2P);
      return group;
    }
    ...
    public void Remove(uint groupHostId)
    {
      ...
      _server.Configuration.Logger?.Debug("Mesh {HostId} torn down and released", group.HostId);
    }
