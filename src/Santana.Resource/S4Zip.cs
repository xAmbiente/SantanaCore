  ...
  public class S4Zip : IReadOnlyDictionary<string, S4ZipEntry>
  {
    ...
    public void Open(byte[] data)
    {
      ...
      using (var r = data.ToBinaryReader())
      {
        ...
        var entryCount = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(S4Zip), ResourceGuard.MaxEntries);
        for (var i = 0; i < entryCount; i++)
        {
          var entrySize = ResourceGuard.EnsureByteCount(r.ReadInt32(), nameof(S4ZipEntry), ResourceGuard.MaxEntryBytes);
          var entryData = r.ReadBytes(entrySize);
          if (entryData.Length != entrySize)
            throw new EndOfStreamException("Invalid s4 league file container entry");
          ...
          using (var entryReader = entryData.ToBinaryReader())
          {
            ...
            var length = ResourceGuard.EnsureByteCount(
                entryReader.ReadInt32(),
                nameof(S4ZipEntry.Length),
                ResourceGuard.MaxDecompressedBytes);
    ...
    public static byte[] DecryptS4(byte[] data)
    {
      ...
      using (var r = data.ToBinaryReader())
      {
        realSize = ResourceGuard.EnsureByteCount(
            r.ReadInt32(),
            nameof(DecryptS4),
            ResourceGuard.MaxDecompressedBytes);
        buffer = r.ReadToEnd();
