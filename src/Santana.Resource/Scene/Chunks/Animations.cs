...
namespace Santana.Resource.Scene.Chunks
{
  public class TransformKeyData2 : TransformKeyData
  {
    ...
    public override void Deserialize(Stream stream)
    {
      ...
      {
        MorphKeys = r.DeserializeArray<MorphKey>(
            ResourceGuard.EnsureCount(r.ReadInt32(), nameof(MorphKeys))).ToList();
      }
    }
  }
  ...
  public class TransformKeyData : IManualSerializer
  {
    ...
    public virtual void Deserialize(Stream stream)
    {
      using (var r = stream.ToBinaryReader(true))
      {
        ...
        FloatKeys = r.DeserializeArray<FloatKey>(
            ResourceGuard.EnsureCount(r.ReadInt32(), nameof(FloatKeys))).ToList();
      }
    }
  }
  ...
  public class MorphKey : IManualSerializer
  {
    ...
    public void Deserialize(Stream stream)
    {
      using (var r = stream.ToBinaryReader(true))
      {
        ...
        var count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Rotations));
        for (var j = 0; j < count; j++)
          Rotations.Add(new Quaternion(r.ReadSingle(), r.ReadSingle(), r.ReadSingle(), r.ReadSingle()));

        count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Positions));
        for (var j = 0; j < count; j++)
