...
namespace Santana.Resource.Scene.Chunks
{
  public class ModelChunk : SceneChunk
  {
    ...
    public override void Deserialize(Stream stream)
    {
      ...
      using (var r = stream.ToBinaryReader(true))
      {
        ...
        WeightData = r.DeserializeArray<WeightData>(
            ResourceGuard.EnsureCount(r.ReadInt32(), nameof(WeightData))).ToList();
        var count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Animation));
  ...
  public class MeshData : IManualSerializer
  {
    ...
    public virtual void Deserialize(Stream stream)
    {
      using (var r = stream.ToBinaryReader(true))
      {
        var count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Vertices));
        for (var i = 0; i < count; i++)
          Vertices.Add(new Vector3(r.ReadSingle(), r.ReadSingle(), r.ReadSingle()));
        count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Faces));
        for (var i = 0; i < count; i++)
          Faces.Add(new Vector3(r.ReadInt16(), r.ReadInt16(), r.ReadInt16()));
        count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Normals));
        for (var i = 0; i < count; i++)
          Normals.Add(new Vector3(r.ReadSingle(), r.ReadSingle(), r.ReadSingle()));
        count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(UV));
        for (var i = 0; i < count; i++)
          UV.Add(new Vector2(r.ReadSingle(), r.ReadSingle()));
        if (ModelChunk.TextureData.Unk2 == 1)
          for (var i = 0; i < count; i++)
            UV2.Add(new Vector2(r.ReadSingle(), r.ReadSingle()));
        count = ResourceGuard.EnsureCount(r.ReadInt32(), nameof(Unk));
        for (var i = 0; i < count; i++)
  ...
  public class WeightData : IManualSerializer
  {
    ...
    public virtual void Deserialize(Stream stream)
    {
      using (var r = stream.ToBinaryReader(true))
      {
        Unk1 = r.ReadCString();
        Matrix = r.ReadMatrix();
        var count = ResourceGuard.EnsureCount(r.ReadUInt32(), nameof(Unk2));
        for (var i = 0; i < count; i++)
  ...
  public class TextureData : IManualSerializer
  {
    ...
    public virtual void Deserialize(Stream stream)
    {
      using (var r = stream.ToBinaryReader(true))
      {
        ...
        if (Unk1 >= 0.2000000029802322f)
          Unk2 = r.ReadInt32();
        var count = ResourceGuard.EnsureCount(r.ReadUInt32(), nameof(Textures));
        for (var i = 0; i < count; i++)
        {
