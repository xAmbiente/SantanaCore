...
namespace SantanaLib.DotNetty.Codecs
{
    public class LengthFieldBasedFrameDecoder : ByteToMessageDecoder
    {
        ...
        {
            var le = order == ByteOrder.LittleEndian;
            long frameLength;
            switch (length)
            {
                case 1:
                    ...
                    break;
                case 2:
                    frameLength = le ? buffer.GetShortLE(offset) : buffer.GetShort(offset);
                    break;
                case 4:
                    frameLength = le ? buffer.GetIntLE(offset) : buffer.GetInt(offset);
                    break;
                case 8:
                    frameLength = le ? buffer.GetLongLE(offset) : buffer.GetLong(offset);
                    break;
                default:
